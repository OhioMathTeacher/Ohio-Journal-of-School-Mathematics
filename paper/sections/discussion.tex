...
Using the \emph{Citation Validator} today requires installing Python, running a command-line script or starting a local Flask web server (Figure~\ref{fig:web-interface-local}).

\FloatBarrier
\begin{figure}[!h]
  \centering
  \caption{The \emph{Citation Validator} web interface running locally.}
  \label{fig:web-interface-local}
  \vspace{0.5\baselineskip}
  \fbox{\includegraphics[width=0.83\textwidth]{figures/fig2-web-interface.png}}
  \vspace{0.75\baselineskip}
  \caption*{%
    \textbf{Note:} Users paste a BibTeX bibliography and receive per-citation results. The interface runs on a local Flask server, which currently requires Python installation.
  }
\end{figure}
\FloatBarrier For a computer scientist, that is five minutes of setup. For a mathematics education journal editor---which is what the first author is---it is a real barrier. For a high school teacher evaluating a student's AI-assisted research paper, it is a wall.
