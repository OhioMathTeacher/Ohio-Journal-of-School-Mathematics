...
\begin{figure}[!h]
  \centering
  \caption{\emph{Citation Validator} output showing all four status categories on a mixed bibliography.}
  \label{fig:validator-output}
  \includegraphics[width=0.85\textwidth]{figures/fig1-validator-output.png}
  \vspace{0.75\baselineskip}
  \caption*{%
    \textbf{Note:} Citations with verified DOIs receive \textit{valid} status (green). Citations absent from databases receive \textit{warning} (yellow). Only \textit{suspicious} and \textit{invalid} statuses count as flags.
  }
\end{figure}
\FloatBarrier
