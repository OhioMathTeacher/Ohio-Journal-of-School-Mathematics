...
The \emph{Citation Validator} accepts a BibTeX bibliography (pasted into a web interface or passed to a command-line script, Figure~\ref{fig:web-interface}) and classifies each citation into one of four statuses:

\FloatBarrier
\begin{figure}[!h]
  \centering
  \caption{The \emph{Citation Validator} web interface.}
  \label{fig:web-interface}
  \vspace{0.5\baselineskip}
  \fbox{\includegraphics[width=0.80\textwidth]{figures/fig1.png}}
  \vspace{0.75\baselineskip}
  \caption*{%
    \textbf{Note:} The interface provides three input methods.
    \ding{192}~\textbf{Benchmark Library:} Pre-loaded datasets of real and fabricated citations, organized by category and filterable by type. Users can load curated test sets to evaluate the validator or explore known hallucination patterns.
    \ding{193}~\textbf{File Upload:} Drag-and-drop or click to upload a \texttt{.bib} file directly.
    \ding{194}~\textbf{Text Entry:} Paste raw BibTeX entries into the editor.
    An optional AI analysis toggle (bottom of interface) enables Tier~3 processing with a configurable provider. The tool is freely hosted at \url{https://huggingface.co/spaces/ojsm/citation-validator}.
  }
\end{figure}
\FloatBarrier

Each citation is classified into one of four statuses: \textit{valid} (verified against databases), \textit{warning} (unverifiable but not flagged), \textit{suspicious} (multiple red flags; flagged), or \textit{invalid} (confirmed fake or failed DOI; flagged). Only \textit{suspicious} and \textit{invalid} citations count as detections. The \textit{warning} status is deliberately not treated as a flag---this distinction is central to the epistemological argument developed in the Discussion.
...
\subsubsection{Reproducibility Pipeline}

All experiments reported in this paper were run using two command-line scripts included in the repository. The false-positive baseline (\texttt{run\_fp\_baseline.py}) runs the validator directly against the real-citation datasets, requiring no server:

\begin{verbatim}
  python3 scripts/run_fp_baseline.py --step 1
\end{verbatim}

\noindent The benchmark runner (\texttt{run\_benchmark.py}) tests fabricated-citation datasets through the full pipeline, with optional AI analysis via a specified provider:
...
\noindent Both scripts load BibTeX files from a dataset manifest (\texttt{datasets/manifest.json}), pass them through the validation pipeline, compare each citation's final status against ground-truth labels, and compute detection rates, false-positive rates, and timing. Every run is logged as a structured JSON record to \texttt{results/experiments/experiment\_log.jsonl}, capturing the dataset, provider, git commit hash, timestamp, per-citation results, and aggregate metrics. This log serves as the primary evidence for the results reported below---any claim in this paper can be traced to a specific, reproducible run.

\FloatBarrier
\begin{figure}[!h]
  \centering
  \caption{Terminal output from a benchmark run against the Ansari~100 dataset.}
  \label{fig:benchmark-output}
  \vspace{0.5\baselineskip}
  \fbox{\includegraphics[width=0.83\textwidth]{figures/fig2.png}}
  \vspace{0.75\baselineskip}
  \caption*{%
    \textbf{Note:} The script loads 100 fabricated citations from the Ansari dataset, runs each through the deterministic pipeline, and reports per-citation status and aggregate detection metrics. Each run is logged with a git commit hash for full traceability.
  }
\end{figure}
\FloatBarrier
