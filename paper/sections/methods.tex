...
If the BibTeX entry contains a DOI field, the tool resolves it through the CrossRef REST API. If CrossRef returns a~404, two fallbacks are attempted in sequence: the DOI.org handle API and content negotiation via DOI.org with CSL-JSON format. ArXiv DOIs (prefix \texttt{10.48550/arXiv.*}) are routed to the arXiv export API for metadata retrieval. All API endpoints are listed in Table~\ref{tab:apis}.
