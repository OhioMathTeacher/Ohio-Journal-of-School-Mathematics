...
If the BibTeX entry contains a DOI field, the tool resolves it through CrossRef (\texttt{api.crossref.org/works/\{doi\}}). If CrossRef returns a 404, two fallbacks are attempted in sequence: the DOI.org handle API (\texttt{doi.org/api/handles/\{doi\}}) and content negotiation via DOI.org with CSL-JSON format. ArXiv DOIs (prefix \texttt{10.48550/arXiv.*}) are routed to the arXiv API (\texttt{export.arxiv.org/api/query}) for metadata retrieval.
...
A DOI that does not resolve at all is strong evidence of fabrication: legitimate DOIs persist indefinitely. A DOI that resolves to a \emph{different} paper (low title similarity, mismatched authors) indicates either a metadata error in the BibTeX entry or a ``stolen DOI''---a real identifier paired with fabricated metadata.
...
\begin{itemize}
  \item \textbf{OpenAlex} \citep{openalex2026}: structured title filter query (\texttt{works?filter=title.search:\{title\}}), returning the top result. A Jaccard similarity $\geq 0.5$ between the query title and the returned title constitutes a strong match; $\geq 0.3$ is noted but not validating.
  \item \textbf{Semantic Scholar} \citep{semanticscholar2026}: free-text title search (\texttt{paper/search?query=\{title\}}), returning title, authors, year, venue, and external identifiers. A strong match requires both title similarity $\geq 0.5$ \emph{and} author/year cross-validation.
\end{itemize}
